...
logging.basicConfig(level=logging.INFO, format="DATA-PERSISTANCE-WEBSERVICE: %(asctime)s [%(levelname)s]: %(message)s")
...
def get_image():
    ...
    file_path=glob.glob(osp.join(data_persistance_config.images_folder,
                                 f'{isic_id_str}*'))
    if len(file_path)==0:
        return jsonify({"error":"image not found"}),500
    return send_file(file_path[0], 
                     as_attachment=True, 
                     download_name=file_path[0].split('/')[-1])
...
def query_records_by_day():
    ...
    try:
        day=datetime.strptime(str(day_str),
                              "%Y%m%d")
    except ValueError:
