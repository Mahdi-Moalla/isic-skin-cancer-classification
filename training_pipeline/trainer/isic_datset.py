...
class isic_skin_cancer_datset(
    ...
    def __getitem__(self, idx):
        ...
        # print(image.shape)
        label = (
            self.metadata.loc[self.isic_id_lut[isic_id], 'target']
        ...
        return (
            ...
            torch.tensor(
                self.metadata.loc[self.isic_id_lut[isic_id], self.tab_features],
                dtype=torch.float,
            ),
            label,
        )
...
class isic_train_sampler(Sampler):  # pylint: disable=invalid-name
    """
